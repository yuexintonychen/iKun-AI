...
Compared with Aiku v1.0 which we had shown in our status report, Aiku v2.0 is mainly focused on optimizing the overall performance of our agent. In fact, the agent can be at most about 11.5 times faster than the previous generation of Aiku. Here are some ways that we have used to achieve such huge performance improvement.
First, we complete redesigned the way our agent interact with our Neural Network Machine Learning Algorithm, which is what we have called "the bridge." By allowing our agent to show what the algorithm had done in real time, our agent now has no need to be put in sleep during each running session of the algorithm.
Second, just like what we have mentioned in the first part, we made our agent sleep as less as possible. That means our agent will now be able to take less rest but do more job. In general, that basically means we defeat an old saying: "You can't expect the horse to run fast when you don't let it graze."
Finally, we have now made use of multi-processing in our agent, which basically means we have moved the time-consuming jobs into seperate processes so that our agent will not get into such "traffic jam" anymore. Of course we cannot just simply use a seperate process since our agent needs to be fed with data for exploring the maze, so we did apply some tricks to achieve this.
